...
class ShapedIndexExpression(Shaped):
    ...
    def _with_newaxes(self, args: tuple[IndexArg, ...]) -> Self:
        ...
        --------
        >>> import jax.numpy as jnp
        >>> from grgg.indexing import ShapedIndexExpression
        >>> shape = (6, 5, 5)
...
class IndexableShape(Shaped):
    ...
    >>> import jax.numpy as jnp
    >>> import numpy as np
    >>> from grgg.indexing import IndexableShape
...
class CartesianCoordinates(Shaped):
    ...
    def __getitem__(
        ...
        coords = jnp.broadcast_arrays(*(joint_mapping[i] for i in range(len(args))))
        return coords
...
class MultiIndexRavel(Shaped):
    ...
    --------
    >>> import jax.numpy as jnp
    >>> from grgg.indexing import MultiIndexRavel
