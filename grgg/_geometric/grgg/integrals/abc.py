...
from grgg.utils.integrate import AbstractIntegral

if TYPE_CHECKING:
    from grgg.models.geometric.grgg import GRGG
    from grgg.models.geometric.grgg._views import NodePairView, NodeView
...
class AbstractNodesIntegral(AbstractModelIntegral):
    ...
    def from_nodes(cls, nodes: "NodeView", *args: Any, **kwargs: Any) -> Self:
        """Construct a model integral from a model."""
        if not isinstance(nodes.model, (typ := AbstractGeometricGraph)):
            errmsg = (
                f"model must be an instance of '{typ.__name__}', "
...
class AbstractPairsIntegral(AbstractModelIntegral):
    ...
    def from_pairs(cls, pairs: "NodePairView", *args: Any, **kwargs: Any) -> Self:
        """Construct a model integral from a model."""
        if not isinstance(pairs.model, (typ := AbstractGeometricGraph)):
            errmsg = (
                f"model must be an instance of '{typ.__name__}', "
